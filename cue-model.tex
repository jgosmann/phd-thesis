...
A second alternative, is to learn the association from $\tcmitem_i \circledast \vc p_i$ to $\tcmitem_{i+1}$ (\cref{fig:hebb-forward}).
\begin{figure}
    \centering
    \begin{tikzpicture}[nef]
        \graph [no placement] {
...
The Semantic Pointer for previous position can be trivially decoded from the position network.
