...
The fuzzy temporal memory is constructed with the aim to represent past values of a function $f(t)$ with a scale-free fall off in accuracy.
...
\begin{equation}
    \tau_i(\nu) = - {(1 + \nu)}^k t^*_i
\end{equation}
and is caused by the discrete approximation of the derivative that relies on $t^*_{i - k}$ to $t^*_{i + k}$ for the reconstruction of $\hat{f}_t(t + t^*_i)$ and is dominated by $t^*_{i + k}$ (TODO appendix).
\begin{figure}
    \centering
...
Based on these equations, the total number of neurons $N_{\ped{tot}}$ required can be estimated as (TODO appendix)
\begin{align}
...
\Textcite{shankar2013} also comment themselves that $t^*_1$ needs to be kept sufficiently far from zero as it introduce a relative error in the order of $\bO(k^3\nu^2/96 t^{*2}_1)$ in the construction of the activity of $c_i$.
