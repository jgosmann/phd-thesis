...
\end{equation}
and is caused by the discrete approximation of the derivative that relies on unequally spaced $t^*_{i - k}$ to $t^*_{i + k}$ for the reconstruction of $\hat{f}_t(t + t^*_i)$ and is dominated by $t^*_{i + k}$ (\cref{apdx:fuzzymem-derivative}).
\begin{figure}
    \centering
...
This parameters space suffers from large relative errors unrelated to noise in the approximation of the inverse Laplace transform with Post's formula and discretized derivative.
