...
\begin{figure}
    ...
    \caption[Mean error along line cut with $\csdist(\dims + 2)$ distributed intercepts]{Mean error along a line cut through the 64-dimensional hyper-ball with 3200 neurons and different intercept distributions. The shaded regions indicate 95\% confidence intervals.}\label{fig:error-cs-intercepts}
\end{figure}
...
Besides pure representation it is common to compute dot products and circular convolutions in the context of the SPA\@.
...
In the general case of non-identical inputs, this might not be fully optimal, but still performs well.
TODO fig

Besides the representation of a vector, the choice of intercept and evaluation point distribution improves the accuracy of the dot product to the level of the radius adjustment method (\cref{fig:spaopt-dot}).
Note that 

Similar results can be expected for more than 
