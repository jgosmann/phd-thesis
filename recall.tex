...
\begin{figure}
    \begin{captionbeside}[Independent accumulator (IA) network]{Independent accumulator (IA) network. The second layer computes $\bar{x}_i = \Heavi(x_i - \vartheta)$.\label{fig:ia}}
        \begin{tikzpicture}[nef, every path/.style={-{Latex}}]
...
Finally, both networks might produce a transient response (\cref{fig:ia-transient}).
...
\begin{figure}
    ...
    \begin{tikzpicture}[nef]
        \graph [no placement] {
            ...
            xib -> ["$x_{\ped{null}}$" {very near end, xshift=3mm}] m2/"" [ens, x=2.5, y=-2] -> r2/"" [rect, x=1.5, y=-2] -> [inhibit, "$\Heavi(x - 0.3)$" {fill=white, inner sep=0cm, yshift=-2mm}] xi;
            r2 -> [recurrent] r2;
            ...
            m3 -> g4/"" [ea, x=3.5, y=-4.5] -> m4/"" [ea, x=2, y=-4.5] -> r4/"" [rect, x=0, y=-4.5] -> ["$-6$" xshift=-3mm, bend left] xi;
            m4 -> [recurrent] m4;
            m4 -> ["$-1$", bend left] g4;
        };
        ...
        \node (freset) [fit=(m2) (r2), draw, dashed, gray, rounded corners=0.5em, inner sep=0.5em] {};
        \node [below=0cm of freset.south west, anchor=north west, gray, font={\lato}] {failed recall reset};

        \node (stage1) [fit=(g3) (m3) (dot) (gs3) (g3) (b3), draw, dashed, gray, rounded corners=0.5em, inner sep=0.5em] {};
        \node [below=0cm of stage1.south west, anchor=north west, gray, font={\lato}] {recall inhibition stage 1};

        \node (stage2) [fit={(m4) (g4) (2, -3.75)}, draw, dashed, gray, rounded corners=0.5em, inner sep=0.5em] {};
        \node [below=0cm of stage2.south west, anchor=north west, gray, font={\lato}] {recall inhibition stage 2};
    \end{tikzpicture}    
    \caption{TODO}\label{fig:recall}
\end{figure}
